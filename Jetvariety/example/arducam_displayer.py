...
from fiducial_tracker import FiducialTracker
import time
import sys
...
def display(cap, arducam_utils, fps = False):
    ...
    # Initialize a FiducialTracker object
    redTrack = FiducialTracker()
    ...
    while True:
        ...
        # Convert to GPU mat for cuda acceleration
        gpuFrame = cv2.cuda_GpuMat(frame)

        # Threshold on red
        redMask = redTrack.gpuGetRedMask(gpuFrame)

        # Find red contours
        centroids = []
        redTrack.findFiducials(redMask,centroids)
        redTrack.updateFiducials(gpuFrame)

        redMask = resize(redMask,640.0)
        frame = resize(frame, 1280.0)
        # display
        cv2.imshow("Arducam", frame)
        cv2.imshow("Red Tracker", redMask)
        ret = cv2.waitKey(1)
        # press 'q' to exit.
        if ret == ord('q'):
            break
